    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{skills.length} skills</p>
        </div>
        <button
      ...
      {editing && (
        <div className="card mb-6">
          <h2 className="text-white font-semibold mb-4">{editingId ? 'Edit skill' : 'New skill'}</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Years used</label>
              <input
                type="number"
          ...
          <div key={skill.id} className="card flex items-center justify-between gap-3">
            <div>
              ...
              <div className="flex gap-2 mt-1">
                ...
                {skill.yearsUsed && (
                  <span className="text-xs text-gray-600">· {skill.yearsUsed}y</span>
                )}
              </div>
            </div>
            <div className="flex gap-2 shrink-0">
              <button
                onClick={() => { setEditing({ ...skill }); setEditingId(skill.id) }}
