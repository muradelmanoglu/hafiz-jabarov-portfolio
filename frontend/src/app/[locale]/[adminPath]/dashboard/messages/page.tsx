    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-2">
          {submissions.length === 0 && <p className="text-gray-600 text-sm">{t('noSubmissions')}</p>}
          {submissions.map((sub) => (
            <div
              key={sub.id}
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={cn('text-xs px-2 py-0.5 rounded-full shrink-0', STATUS_COLORS[sub.status] || 'bg-gray-800 text-gray-400')}>
                      {sub.status}
                    </span>
                  </div>
