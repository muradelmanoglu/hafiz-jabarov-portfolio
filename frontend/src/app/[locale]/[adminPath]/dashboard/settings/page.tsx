...
type AllTranslations = { az: TranslationMap; ru: TranslationMap }

const TRANSLATABLE_FIELDS = [
  'siteTitle', 'tagline', 'heroHeadline', 'heroSubheadline',
  'metaDescription', 'availabilityMessage', 'copyrightText', 'colophonText',
] as const
...
export default function SettingsPage() {
  ...
  const [translations, setTranslations] = useState<AllTranslations>({ az: {}, ru: {} })
  ...
  useEffect(() => {
    adminApi.getSettings().then((res) => {
      if (res.data.data) {
        ...
        setSettings(s)
        try {
          const parsed = JSON.parse(s.settingsTranslationsJson || '{}')
          setTranslations({ az: parsed.az || {}, ru: parsed.ru || {} })
        } catch { /* empty */ }
      }
    })
  }, [])
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: Partial<SiteSettings> = {
        ...settings,
        settingsTranslationsJson: JSON.stringify(translations),
      }
  ...
  // Get/set value depending on active lang tab
  const getVal = (key: typeof TRANSLATABLE_FIELDS[number]): string => {
    if (langTab === 'en') return (settings[key] as string) || ''
    return translations[langTab][key] || ''
  }
  const setVal = (key: typeof TRANSLATABLE_FIELDS[number], val: string) => {
    if (langTab === 'en') {
      setSettings({ ...settings, [key]: val })
    } else {
      setTranslations({ ...translations, [langTab]: { ...translations[langTab], [key]: val } })
    }
  }

  // EN-only field (contact/social — not translatable)
  const enField = (key: keyof SiteSettings, label: string, textarea?: boolean) => (
    <div>
  ...
  // Translatable field — shows value for active lang tab
  const trField = (key: typeof TRANSLATABLE_FIELDS[number], label: string, textarea?: boolean) => (
    <div>
      <label className="block text-xs text-gray-500 mb-1">{label}</label>
      {textarea ? (
        <textarea
      ...
      <div className="space-y-6">
        ...
        <div className="card">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {trField('siteTitle', tf('siteTitle'))}
            {trField('tagline', tf('tagline'))}
            {trField('heroHeadline', tf('heroHeadline'))}
            {trField('metaDescription', tf('metaDescription'), true)}
            {trField('heroSubheadline', tf('heroSubheadline'), true)}
          </div>
        </div>
        ...
        <div className="card">
          ...
          <LangTabs />
          <div className="grid sm:grid-cols-2 gap-4">
            {trField('availabilityMessage', tf('availabilityMessage'))}
          </div>
        </div>

        {/* Contact & Social — NO language tabs */}
        <div className="card">
          <h2 className="text-white font-semibold mb-4 text-sm uppercase tracking-widest" style={{ color: 'var(--accent)' }}>
            {t('contactSocial')}
          </h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {enField('email', tf('email'))}
            {enField('phone', tf('phone'))}
            {enField('linkedIn', tf('linkedIn'))}
            {enField('github', tf('github'))}
            {enField('calendly', tf('calendly'))}
            {enField('twitter', tf('twitter'))}
            {enField('instagram', tf('instagram'))}
        ...
        <div className="card">
          ...
          <p className="text-gray-500 text-xs mb-4">Add any extra social platform (e.g. YouTube, Dribbble, TikTok).</p>
          <div className="space-y-2 mb-4">
            {parsedSocialLinks.map((link, i) => (
              <div key={i} className="flex items-center gap-3 bg-gray-800 rounded-lg px-3 py-2">
                ...
                <button onClick={() => { const arr = [...parsedSocialLinks]; arr.splice(i, 1); setSocialLinks(arr) }} className="text-gray-600 hover:text-red-400 shrink-0"><X size={14} /></button>
              </div>
            ))}
          </div>
          <div className="flex gap-2 items-center">
            <input value={newSocialLink.label} onChange={(e) => setNewSocialLink({ ...newSocialLink, label: e.target.value })} placeholder="Platform (e.g. YouTube)" className="admin-input text-sm w-40 shrink-0" />
            <input value={newSocialLink.url} onChange={(e) => setNewSocialLink({ ...newSocialLink, url: e.target.value })} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); if (!newSocialLink.label.trim() || !newSocialLink.url.trim()) return; setSocialLinks([...parsedSocialLinks, { ...newSocialLink }]); setNewSocialLink({ label: '', url: '' }) } }} placeholder="https://..." className="admin-input text-sm flex-1" />
            <button onClick={() => { if (!newSocialLink.label.trim() || !newSocialLink.url.trim()) return; setSocialLinks([...parsedSocialLinks, { ...newSocialLink }]); setNewSocialLink({ label: '', url: '' }) }} className="btn-outline px-3 text-sm shrink-0"><Plus size={14} /></button>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <p className="text-gray-500 text-xs mb-4">Shown in the project type dropdown. Leave empty to use defaults.</p>
          <div className="space-y-2 mb-4">
        ...
        <div className="card">
          ...
          <p className="text-gray-500 text-xs mb-4">Shown in the budget range dropdown. Leave empty to use defaults.</p>
          <div className="space-y-2 mb-4">
        ...
        <div className="card">
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            {uploadField('logoUrl', tf('logoUrl'))}
            {uploadField('faviconUrl', tf('faviconUrl'))}
            {uploadField('defaultOgImageUrl', tf('defaultOgImageUrl'))}
          </div>
          <LangTabs />
          <div className="grid sm:grid-cols-2 gap-4">
            {trField('copyrightText', tf('copyrightText'))}
            {trField('colophonText', tf('colophonText'))}
          </div>
        </div>
